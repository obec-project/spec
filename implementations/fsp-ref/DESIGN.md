...
| D48 | **Identity of the Operator's channel.** Every Operator act is made *as* a binding. `fsp`: the Operator id configured for the entity on the host (`~/.fsp/config.json`, Phase 10; until then `--operator`). Adapter: optional `--operator ID` (§2.4 allows extra flags), **default = the founding binding recorded in the Genesis** (ADAPTER.md §3.3, G19). If that binding is not active, the act is refused (`check: binding`). The set is never empty: removing the last active binding is refused (OC-002(a)), so an Operator act always has a binding to be made as. The start is also made as a binding: the credential's `binding` (D46) and that of the `session-open` record. |
