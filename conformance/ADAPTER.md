...
**Version:** suite 0.1.0 (draft) · targets OBEC-Core 0.9.0
...
purposes **the adapter is part of the implementation**. HC-002(b) is violated by
...
|---|---|---|
| `operator` | acts **as the Operator** | Legitimate. HC-002(a) requires that binding changes and other Operator acts be performed "through means the implementation provides directly to the Operator", which is precisely what the adapter is exercising. Operator acts are outside the entity's operation set by design. |
...
- **`refusal`** — present whenever `outcome` is `refused`. **`check` and `logged`
  are the load-bearing fields**: HC-008(d) requires every rejection to carry the
...
| Command | Returns |
|---|---|
| `describe operations` | the operation set. Each entry: `name`, `reachable_from_reasoning` (bool), `writes_classes` (list of `structural`/`mnemonic`/`integrity`, possibly empty), `host_access` (bool), `irreversible` (bool). **HC-001(a), HC-005(b)** |
| `describe state` | every datum the implementation persists, each with the content class it belongs to and whether it lives inside the Entity Store. **HC-003(a)** |
| `describe config` | the configuration surface: every setting, its default, its permitted range, and what it governs. **HC-010** — the suite searches this for any setting that could skip, reorder or downgrade a start gate |
| `describe boundaries` | for each boundary the specification requires, the mechanism that crosses it and whether that mechanism is loggable. **HC-009** |
| `describe inference-channel` | how the model is reached, and the implementation's documented basis for concluding the channel exercises no host authority of its own. **HC-006** |
| `describe context-sources` | every source of content that can enter assembled context, and which of them are persisted. **HC-007** |
...
|---|---|---|
| `lifecycle init --store PATH --operator NAME` | first activation on an empty store | HC-004(a) |
| `lifecycle start --store PATH` | attempt a start. Returns `gates`: an ordered list of `{gate, outcome, rule}`, and `credential_issued` | HC-010, HC-002(a) |
| `lifecycle stop --store PATH` | normal close | HC-003(d) |
| `lifecycle verify --store PATH` | verify structural content against the baseline and the chain back to the Genesis Anchor. Returns `chain_intact`, `content_matches`, `genesis_digest`, `head_digest`, `entry_count` | HC-003(b), HC-004(a) |
| `lifecycle decommission --store PATH --disposition destroy\|archive` | terminal end | HC-001(c) |
| `lifecycle migrate --store PATH --to VERSION` | attempt a version transition | HC-004(c) |
...
HC-003(b) regardless of what else it does.
...
| Command | Purpose | Tests |
|---|---|---|
| `operator binding add --store PATH --id ID` | add a binding | HC-002(a) |
| `operator binding remove --store PATH --id ID` | remove a binding, including the last one | HC-002(a) |
| `operator binding list --store PATH` | the active binding set | HC-002(a) |
| `operator approve --store PATH --proposal ID` | record a per-proposal approval | HC-002(b) |
| `operator grant --store PATH --expiry T --budget N --scope S` | open a standing grant | HC-002(b) |
| `operator revoke-grant --store PATH` | withdraw a standing grant | HC-002(b) |
| `operator set-workspace --store PATH --path P` | declare the workspace boundary | HC-008(a), HC-008(b) |
| `operator set-rule --store PATH --target T --mode allow\|deny\|hold` | operational rules | HC-008 |
| `operator revoke-credential --store PATH [--direct]` | revoke the session credential. `--direct` removes the credential artifact with no component's cooperation, as an Operator would | HC-001(b), HC-005(c) |
| `operator clear-passive-signal --store PATH` | clear a passive signal | HC-002(c) |
...
permissive implementation choice — HC-002(b) requires all three simultaneously.
...
the only mechanical test of HC-001(b) available.
...
| Command | Purpose | Tests |
|---|---|---|
| `entity propose --store PATH --ops FILE` | originate an evolution proposal | HC-002(b) |
| `entity commit --store PATH --proposal ID` | attempt to commit a proposal under whatever authorization currently exists | HC-002(b), HC-004(b) |
| `entity actuate --store PATH --op read\|write\|execute --target P` | attempt a host operation | HC-008(a), HC-008(b) |
| `entity invoke-skill --store PATH --name N` | attempt a skill execution | HC-008(c) |
| `entity attempt-write --store PATH --class structural\|mnemonic\|integrity --via OPERATION --target T` | attempt to write a content class through an operation that does not own it, including by parameter manipulation | HC-003(c), HC-005(b) |
| `entity stimulate --store PATH --text S` | deliver a stimulus | HC-007 |
...
| Command | Returns |
|---|---|
| `observe log --store PATH [--since ID] [--kind K]` | integrity log records, each with `id`, `kind`, `rule`, and — for records of Operator acts — the `binding` that produced it. **HC-002(a)** requires the attribution; the suite samples records and asserts it |
| `observe memory --store PATH --sample N` | mnemonic records with the session each is attributable to. **HC-003(e)** |
| `observe chain --store PATH` | chain entries in order: `id`, `predecessor`, `state_digest`, `authorization`, and `version` where a version-transition entry. **HC-004** |
...
|---|---|---|
| `inject interrupt --store PATH --stage staging\|write\|chain-entry` | terminate a commit mid-pipeline | HC-004(b) |
| `inject gate-failure --store PATH --gate G` | cause one start gate to fail | HC-010 |
| `inject passive-signal --store PATH` | write a passive signal without waiting for `N_channel` delivery failures | HC-002(c), HC-010 |
| `inject corrupt --store PATH --kind structural-byte\|chain-entry-removed\|chain-entry-forged\|commit-unauthorized\|skill-file` | targeted corruption of the implementation's own store | HC-004(a), HC-008(c) |
| `inject probe --store PATH --content FILE` | run the deterministic probe layer against supplied content without consolidating it | HC-001(d) |
...
**Attested tests are reviewed, not executed.** HC-006 and HC-009 are properties
...
HC-010's test is a search of the configuration surface, not only a sequence of
starts.
