...
| `--only HC-003,HC-008` | run a subset |
...
|---|---|---|
| `hc002b` | commits without checking authorization | HC-002 |
| `hc003b` | folds the hostname into verification | HC-003 — **only with a real `--adapter-b`** |
| `hc008a` | skips the workspace boundary check | HC-008 |
| `hc008d` | refuses without naming the check | HC-001, HC-002, HC-003, HC-005, HC-008 |
| `hc010` | issues a credential on a failed gate | HC-002, HC-004, HC-010 |

`hc008d` failing five tests at once is the point of the refusal triple: a
